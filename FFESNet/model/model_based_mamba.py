...
class ModelBasedMamba(nn.Module):
    ...
    def __init__(self, model_backbone_type = 'B0', feature_fuse='LP', prediction='linear'):
        ...
        # Feature extractor
        # ----------------------------------------------------------------
        if self.model_backbone_type == 'T':
            self.backbone = AutoModel.from_pretrained("nvidia/MambaVision-T-1K", trust_remote_code=True)
            dim = 80
        if self.model_backbone_type == 'T2':
            self.backbone = AutoModel.from_pretrained("nvidia/MambaVision-T2-1K", trust_remote_code=True)
            dim = 80
        if self.model_backbone_type == 'S':
            self.backbone = AutoModel.from_pretrained("nvidia/MambaVision-S-1K", trust_remote_code=True)
            dim = 96
        if self.model_backbone_type == 'B':
            self.backbone = AutoModel.from_pretrained("nvidia/MambaVision-B-1K", trust_remote_code=True)
            dim = 128
        if self.model_backbone_type == 'L':
            self.backbone = AutoModel.from_pretrained("nvidia/MambaVision-L-1K", trust_remote_code=True)
            dim = 196
        if self.model_backbone_type == 'L2':
            self.backbone = AutoModel.from_pretrained("nvidia/MambaVision-L2-1K", trust_remote_code=True)
            dim = 196

        self.embed_dims = [dim, dim*2, dim*4, dim*8]

        # self.transform = create_transform(
        #                                     input_size=(3, 352, 352),
        #                                     is_training=True,
        #                                     mean=self.backbone.config.mean,
        #                                     std=self.backbone.config.std,
        #                                     crop_mode=self.backbone.config.crop_mode,
        #                                     crop_pct=self.backbone.config.crop_pct
        #                                  )
        ...
        if feature_fuse == "LP":
            self.ff_1 = mlp.LP(
                                input_dim = self.embed_dims[0],
                                embed_dim = self.embed_dims[0]
                              )
            self.ff_2 = mlp.LP(
                                input_dim = self.embed_dims[1],
        ...
        self.linear_fuse34 = ConvModule(
                                        in_channels=(self.embed_dims[2] + self.embed_dims[3]),
        ...
        self.linear_fuse23 = ConvModule(
                                        ...
                                        out_channels=self.embed_dims[1],
                                        kernel_size=1,
                                        norm_cfg={'type':'BN', 'requires_grad':True}
                                       )
        self.linear_fuse12 = ConvModule(
                                        in_channels=(self.embed_dims[0] + self.embed_dims[1]),
        ...
        # ----------------------------------------------------------------
        self.aux_conv1 = Conv(3, self.embed_dims[0], 3, 2)
        self.aux_conv2 = Conv(self.embed_dims[0], self.embed_dims[1], 3, 2)
        ...
        self.adown1 = ADown(self.embed_dims[1], self.embed_dims[1])
        ...
        self.adown2 = ADown(self.embed_dims[2], self.embed_dims[2])
        ...
        self.adown3 = ADown(self.embed_dims[3], self.embed_dims[3])
    ...
    def forward(self, x):
        ...
        # out_1 = out_1.reshape(b, h, w, -1).permute(0, 3, 1, 2).contiguous()  #(Batch_Size, self.embed_dims[0], 88, 88)
        ...
        # out_2 = out_2.reshape(b, h, w, -1).permute(0, 3, 1, 2).contiguous()  #(Batch_Size, self.embed_dims[1], 44, 44)
        ...
        # out_3 = out_3.reshape(b, h, w, -1).permute(0, 3, 1, 2).contiguous()  #(Batch_Size, self.embed_dims[2], 22, 22)
        ...
        # out_4 = out_4.reshape(b, h, w, -1).permute(0, 3, 1, 2).contiguous()  #(Batch_Size, self.embed_dims[3], 11, 11)
